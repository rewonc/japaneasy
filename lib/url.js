var url = function(config){
  var dictionary, method, encoding, custom, default_mirror, timeout, last;
  config = config || {};
  ...
  if (config.mirror){
    ...
    default_mirror = mapMirror(config.mirror);
  } else {default_mirror = "http://www.edrdg.org/cgi-bin/wwwjdic/wwwjdic?"}
  ...
  if (custom) return default_mirror + custom;
  return default_mirror + dictionary + "Z" + method + last;
